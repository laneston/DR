...
class DigitTrainer:
    def __init__(self, batch_size, mnist_path):
        if (batch_size > 0)&(batch_size < 65535)&(batch_size % 8 == 0):
            ...
            raise TypeError(f"{batch_size} is not in compliance with regulations, should be rang 0 to 65535.")

        if type(mnist_path)==str:
            self.__mnist_path = mnist_path
        else:
            raise TypeError(f"{mnist_path} is not in compliance with regulations, should be string format.")
    ...
    def train(self, cycles):
        ...
        for cycle in range(1, cycles + 1):
            for batch_idx, (data, target) in enumerate(self.__train_loader): 
                ...
                if batch_idx % 100 == 0:
                    print(f'Train Epoch: {cycle} [{batch_idx * len(data)}/{len(self.__train_loader.dataset)}'
        ...
        for cycle in range(1, cycles + 1):
            ...
            with torch.no_grad():
                for data, target in self.__test_loader:
                    ...
                    test_loss += self.__criterion(output, target).item() * data.size(0)  # 累加批次总损失
                    pred = output.argmax(dim=1, keepdim=True)
                    correct += pred.eq(target.view_as(pred)).sum().item()
            
            test_loss /= len(self.__test_loader.dataset)
            accuracy = 100. * correct / len(self.__test_loader.dataset)
    ...
    def prediction(self, nums):
        if (nums >= 0)&(nums < 10):            
            ...
            raise TypeError(f"{nums} is not in compliance with regulations, should be rang 0 to 9.")
